...
*** Test Cases ***
TC_020 Login with valid credential
  [Tags]    Login
  ${body}  Create Dictionary  email=${LOGIN_EMAIL_4}  password=12345678
...
Update accompany services
  [Tags]    Cart
  ${cart_info}  create list
	${accompany_info}  Create Dictionary  accompany_type=service 	accompany_id=3  external_type=option  external_id=2  note=some note
  append to list  ${cart_info}  ${accompany_info}
  ${body}  Create Dictionary  accompanies=${cart_info}  csrf_token=sdgasdfgadgdae
	${response}=  Patch On Session  ${mysession}  /web/cart/accompany_services    headers=${header}    json=${body}
	Log  ${response.content}
	Log To Console  ${response.content}
	Status Should Be	200	${response}

Update Invoice Request Info
  [Tags]    Cart
  ${body}  Create Dictionary  csrf_token=fgsgsfgsjgnodfg 		email=${LOGIN_EMAIL_4}  code=1500365748  name=CTY TNHH ABC  address=123 Duong abc, P.ABC
	${response}=  Patch On Session  ${mysession}  /web/cart/invoice    headers=${header}    json=${body}
	Log  ${response.content}
	Log To Console  ${response.content}
	Status Should Be	200	${response}
